  ...
  const CartItem = ()=>{
    ...
    cartData.map((i) => {
      ...
      <div class="max-w-sm bg-white rounded-lg shadow grid content-between p-5">
        <button onClick={close} className="float top-0 right-0">X</button><br/>
        <div>
  ...
  return (
    ...
    {ReactDOM.createPortal(
      <div className="bg-transparent">
        ...
        >
          {cartData ? {CartItem} : <div class="bg-white rounded-lg shadow p-5"> <button onClick={close} className="float top-0 right-0">X</button><br/>Please Add item to Cart</div>}
        </div>
      </div>,
